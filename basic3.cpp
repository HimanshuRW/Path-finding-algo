...
void explore(int i, int j)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        Coordinate current_neighbour_index = neighbours_indexes[i];
        ...
        if (!(current_neighbour->wall || current_neighbour->visited))
        {
            ...
            ValueSet set = get_valuset_by_index(i, board[i][j]);
            // cout << "set.value : " << set.value << ",  dir : " << set.direction << endl;
...
void trace()
{
    Coordinate end_index = pixel_to_index(end_pt_x-1, end_pt_y);
    GotoXY(end_pt_x-1,end_pt_y);
    print_visited();
    GotoXY(0,0);
    cout << "value is : "<<board[end_index.x][end_index.y].value;
    exit(0);
}